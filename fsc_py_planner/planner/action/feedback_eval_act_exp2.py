...
class FeedbackEvalAct(Action):
    ...
    def init(self):
        time.sleep(2)
        if self.robot.scenario.correct_edges[int(self.robot.current_ques)] == -1:
            say_text = "no! this is inappropriate choice again. actually there is no correct choice in here. this is a bit tricky!"
            feedback_eval_state = -1
        elif (int(self.robot.selected_ques_ind)-1) == self.robot.scenario.correct_edges[int(self.robot.current_ques)]:
        ...
        selected_ans_mist = self.robot.scenario.all_possible_edges[int(self.robot.current_ques)][int(self.robot.selected_ques_ind)-1]
        self.robot.feedback_eval_states[int(self.robot.current_ques)] = selected_ans_mist
        print(say_text)
        self.robot.say_text(say_text)
        if self.robot.session == "robot":
